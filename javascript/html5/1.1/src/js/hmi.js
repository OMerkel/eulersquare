...
Hmi.prototype.dragmove = function (dx, dy, x, y, e) {
	...
	this.tile[k].inner.attr({
		x: this.newTransform.x + 0.25,
		y: this.newTransform.y + 0.25,
	});
...
Hmi.prototype.dragend = function (e) {
	...
	var tmp = {
		x:
			this.newTransform.y >= size + 0.5
				? this.newTransform.x
				: Math.round(this.newTransform.x),
		y:
			this.newTransform.y > size && this.newTransform.y < size + 0.5
				? size + 0.5
				: this.newTransform.y >= size + 0.5
					? this.newTransform.y
					: Math.round(this.newTransform.y),
	};
	tmp = {
		...
		y:
			tmp.y < 0
				? -1.0
				: tmp.y > Math.floor(size * this.aspect)
					? Math.floor(size * this.aspect)
					: tmp.y,
	};
	var k = this.dragElement.data("id");
...
Hmi.prototype.resize = function () {
	...
	for (var k in this.tile) {
		this.tile[k].board.attr({
			opacity:
				this.tile[k].initial.y >= size || this.tile[k].initial.x >= size
					? 0.0
					: 1.0,
		});
	...
	this.size =
		availableWidth / availableHeight < 1 / this.aspect
...
Hmi.prototype.initGame = function () {
	...
	this.paper
		.rect(-0.5, -0.5, size + 1.0, size * this.aspect + 1.0)
		.attr({
			stroke: "none",
			"stroke-width": 0.0,
			"stroke-linecap": "round",
