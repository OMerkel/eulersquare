...
((root, doc, factory) => {
	...
	(($, undefined) => {
		...
		if ($("<a>").data("a-b", "a").removeData("a-b").data("a-b")) {
			$.fn.removeData = ((removeData) => function (key) {
					if (arguments.length) {
	...
	(($, window, undefined) => {
		...
		var compensateToolbars = (page, desiredHeight) => {
						...
						widgetOptions =
							$.mobile.toolbar && theElement.data("mobile-toolbar")
								...
								: {
										...
										updatePagePadding:
											theElement.attr(
												"data-" + $.mobile.ns + "update-page-padding",
											) !== false,
									};
		...
		$.addDependents = (elem, newDependents) => {
			var $elem = $(elem),
				...
				$.each($.mobile.widgets, (name, constructor) => {
					...
					if (constructor.initSelector) {
						...
						var elements = $.mobile.enhanceable(
							this.find(constructor.initSelector),
						);
		...
		$.removeWithDependents = (nativeElement) => {
			var element = $(nativeElement);

			(element.jqmData("dependents") || $()).remove();
			element.remove();
		};
		$.addDependents = (nativeElement, newDependents) => {
			var element = $(nativeElement),
		...
		$.find.matches = (expr, set) => $.find(expr, null, null, set);

		$.find.matchesSelector = (node, expr) => $.find(expr, null, null, [node]).length > 0;
	})(jQuery, this);
		...
		$.widget.extend = function (target) {
			...
			for (; inputIndex < inputLength; inputIndex++) {
				for (key in input[inputIndex]) {
					...
					if (Object.hasOwn(input[inputIndex], key) && value !== undefined) {
						// Clone objects
	...
	(($, window, undefined) => {
		...
		$.jqmRemoveData = (elem, prop) => $.removeData(elem, $.mobile.nsNormalize(prop));
	...
	(($, undefined) => {
		var rcapitals = /[A-Z]/g,
			replaceFunction = (c) => "-" + c.toLowerCase();
		...
		$.widget("mobile.loader", {
			...
			fakeFixLoader: function () {
				...
				this.element.css({
					top:
						($.support.scrollTop &&
							this.window.scrollTop() + this.window.height() / 2) ||
			...
			show: function (theme, msgText, textonly) {
				...
				message =
					msgText || (loadSettings.text === false ? "" : loadSettings.text);
  ...
  */
		...
		fake_onhashchange = (() => {
			...
			window.attachEvent &&
				...
				(() => {
					...
					history_get = () => get_fragment(iframe.location.href);

					// Set a new history item by opening and then closing the Iframe
					// document, *then* setting its location.hash. If document.domain has
					// been set, update that as well.
					history_set = (hash, history_hash) => {
						...
						if (hash !== history_hash) {
							...
							domain &&
								iframe_doc.write(
									'\x3cscript>document.domain="' + domain + '"\x3c/script>',
								);
	...
	(($, undefined) => {
		...
		$.mobile.media = (q) => window.matchMedia(q).matches;
	})(jQuery);
	...
	(($, undefined) => {
		...
		$.event.special.navigate = self = {
			...
			isHashChangeEnabled: () => $.mobile.hashListeningEnabled === true,
	...
	(($, undefined) => {
		...
		$.mobile.path = path = {
			...
			getDocumentUrl: (asParsedObject) => asParsedObject
			...
			isAbsoluteUrl: (url) => path.parseUrl(url).protocol !== "",
			...
			stripQueryParams: (url) => url.replace(/\?.*$/, ""),

			//remove the preceding hash, any query params, and dialog notations
			cleanHash: (hash) => path.stripHash(
					hash.replace(/\?.*$/, "").replace(dialogHashKey, ""),
				),

			isHashValid: (hash) => /^#[^#]+$/.test(hash),
			...
			squash: function (url, resolutionUrl) {
				...
				if (isPath) {
					...
					if (
						...
						preservedHash.replace("#", "").indexOf(this.uiStateKey) === 0
					) {
						preservedHash = "";
					}
			...
			// request
			getFilePath: (path) => path && path.split(dialogHashKey)[0],
		...
		path.documentBase = $base.length
			? path.parseUrl(
					path.makeUrlAbsolute($base.attr("href"), path.documentUrl.href),
				)
			: path.documentUrl;
		...
		path.getDocumentBase = (asParsedObject) => asParsedObject
	...
	(($, undefined) => {
		...
		$.mobile.navigate = (url, data, noEvents) => {
			...
			if (
				...
				if (fallbackTime === undefined) {
					...
					if ($(this).context !== document) {
						...
						duration =
							parseFloat($(this).css(props[animationType].duration)) * 3000;
					}
				...
				timer = setTimeout(() => {
					$(this).off(props[animationType].event, eventBinding);
					callback.apply(this);
				}, duration);
	...
	(($, undefined) => {
		...
		$.widget("mobile.page", {
			...
			_create: function () {
				...
				if (
					$.mobile.getAttribute(this.element[0], "role") === "dialog" &&
					$.mobile.dialog
				) {
					this.element.dialog();
				}
				...
				this.element
					...
					.each(function () {
						...
						self.options.contentTheme =
							...
							(self.element.jqmData("role") === "dialog" && self.options.theme);
	...
	(($, undefined) => {
		$.widget("mobile.pagecontainer", {
			...
			_getMinScroll: () => $.mobile.minScrollBack,

			_getDefaultScroll: () => $.mobile.defaultHomeScroll,
			...
			_getHash: () => $.mobile.path.parseLocation().hash,
			...
			_getInitialContent: () => $.mobile.firstPage,

			// TODO each content container should have a history object
			_getHistory: () => $.mobile.navigate.history,
			...
			_getDocumentBase: () => $.mobile.path.documentBase,
			...
			_handleDestination: function (to) {
				...
				if (to) {
					...
					to = !$.mobile.path.isPath(to)
						? $.mobile.path.makeUrlAbsolute("#" + to, this._getDocumentBase())
						: to;
				}
			...
			_transitionFromHistory: function (direction, defaultTransition) {
				var history = this._getHistory(),
					entry =
						direction === "back" ? history.getLast() : history.getActive();
			...
			_isRewritableBaseTag: () => $.mobile.dynamicBaseEnabled && !$.support.dynamicBaseTag,

			_createDataUrl: (absoluteUrl) => $.mobile.path.convertUrlToDataUrl(absoluteUrl),

			_createFileUrl: (absoluteUrl) => $.mobile.path.getFilePath(absoluteUrl),
			...
			transition: function (toPage, triggerData, settings) {
				...
				isDialog =
					(settings.role === "dialog" || toPage.jqmData("role") === "dialog") &&
			...
			_findBaseWithDefault: function () {
				var closestBase =
					this.activePage && $.mobile.getClosestBaseUrl(this.activePage);
	...
	(($, undefined) => {
		...
		$.mobile._registerInternalEvents = () => {
			var getAjaxFormData = ($form, calculateOnly) => {
				var url,
					ret = true,
					formData,
					vclickedName,
				...
				if (
					...
					!$.mobile.path.isPermittedCrossDomainRequest(documentUrl, url)
				) {
					return false;
				}
			...
			$.mobile.document.bind("vclick", (event) => {
				...
				if ($.data(target, "mobile-button")) {
					...
					if (
						!(
							target &&
							$.mobile.path.parseUrl(target.getAttribute("href") || "#")
								.hash !== "#"
						)
					) {
						return;
			...
			$.mobile.document.bind("click", (event) => {
				...
				isExternal =
					...
					($.mobile.path.isExternal(href) &&
						!$.mobile.path.isPermittedCrossDomainRequest(documentUrl, href));
		...
		$.when(domreadyDeferred, $.mobile.navreadyDeferred).done(() => {
			$.mobile._registerInternalEvents();
		});
	})(jQuery);
	...
	(($) => {
		...
		var defaultGetMaxScrollForTransition = () => $.mobile.getScreenHeight() * 3;
		...
		$.mobile._maybeDegradeTransition = (transition) => {
			if (
				transition &&
		...
		$.mobile.getMaxScrollForTransition =
			$.mobile.getMaxScrollForTransition || defaultGetMaxScrollForTransition;
	})(jQuery);
	...
	(($, undefined) => {
		...
		$.mobile.degradeInputsWithin = (target) => {
			target = $(target);
	...
	(($, undefined) => {
		var rInitialLetter = /([A-Z])/g,
			...
			iconposClass = (iconpos) => "ui-btn-icon-" + (iconpos === null ? "left" : iconpos);
		...
		$.widget("mobile.collapsible", {
				...
				this._on(ui.heading, {
					...
					click: function (event) {
						this._handleExpandCollapse(
							!ui.heading.hasClass("ui-collapsible-heading-collapsed"),
						);
						event.preventDefault();
						event.stopPropagation();
					},
			...
			_themeClassFromOption: (prefix, value) => value ? (value === "none" ? "" : prefix + value) : "",
			...
			$.extend(
				{
					...
					_create: function () {
						...
						listviewClasses += this.options.inset ? " ui-listview-inset" : "";

						if (this.options.inset) {
							listviewClasses += this.options.corners ? " ui-corner-all" : "";
							listviewClasses += this.options.shadow ? " ui-shadow" : "";
						}

						// create listview markup
						this.element.addClass(" ui-listview" + listviewClasses);

						this.refresh(true);
					},

					// TODO: Remove in 1.5
					_findFirstElementByTagName: (ele, nextProp, lcName, ucName) => {
						var dict = {};
					...
					_getChildrenByTagName: (ele, lcName, ucName) => {
						var results = [],
							dict = {};
						dict[lcName] = dict[ucName] = true;
						ele = ele.firstChild;
					...
					refresh: function (create) {
						var buttonClass,
							...
							countTheme =
								getAttr($list[0], "counttheme") || this.options.countTheme,
						...
						for (pos = 0, numli = li.length; pos < numli; pos++) {
							...
							if (
								create ||
								item[0].className.search(/\bui-li-static\b|\bui-li-divider\b/) <
									0
							) {
								...
								if (
									...
									dividerTheme =
										getAttr(item[0], "theme") || o.dividerTheme || o.theme;
			...
			$.extend(
				{
					...
					_create: function () {
						...
						(o.iconpos =
							...
							label.element.attr("data-" + $.mobile.ns + "iconpos") ||
							o.iconpos),
							// Establish options
					...
					_handleLabelVClick: function (event) {
						...
						input.prop(
							"checked",
							(this.inputtype === "radio" && true) || !input.prop("checked"),
						);
					...
					_updateAll: function (changeTriggered) {
						...
						this._getInputSet()
							.each(function () {
								...
								if (
									(this.checked || self.inputtype === "checkbox") &&
									!changeTriggered
								) {
									$this.trigger("change");
								}
								...
								return (
									(controlgroupWidget
										...
										: // ... otherwise decide based on the "type" data attribute.
											controlgroup.attr("data-" + $.mobile.ns + "type")) !==
									"horizontal"
								);
							}
						}
	...
	(($, undefined) => {
		$.widget("mobile.button", {
			...
			_getIconClasses: (options) => options.icon
					? "ui-icon-" +
							options.icon +
							(options.iconshadow
								? " ui-shadow-icon"
			...
			_setOptions: function (options) {
				...
				if (
					...
					outer
						...
						.addClass(
							this._getIconClasses($.extend({}, this.options, options)),
						);
				}
					...
					_create: function () {
						...
						var 
							...
							trackThemeClass = trackTheme
								...
								: " ui-bar-inherit",
							cornerClass =
								this.options.corners || control.jqmData("corners")
									? " ui-corner-all"
									: "",
							miniClass =
								this.options.mini || control.jqmData("mini") ? " ui-mini" : "",
					...
					refresh: function (val, isfromControl, preventInputUpdate) {
						...
						if (
							...
							this.valuebg = (() => {
								var bg = document.createElement("div");
								bg.className = "ui-slider-bg " + $.mobile.activeBtnClass;
								return $(bg).prependTo(this.slider);
							})();
						}
						...
						this.handle[0].setAttribute(
							"aria-valuenow",
							isInput ? newval : optionElements.eq(newval).attr("value"),
						);

						this.handle[0].setAttribute(
							"aria-valuetext",
							isInput ? newval : optionElements.eq(newval).getEncodedText(),
						);
	...
	(($, undefined) => {
		...
		$.widget("mobile.slider", $.mobile.slider, {
			...
			_positionPopup: function () {
				...
				this._popup.offset({
					left:
						dstOffset.left + (this.handle.width() - this._popup.width()) / 2,
			...
			$.extend(
				{
					...
					_toggle: function () {
						var direction = this.flipswitch.hasClass("ui-flipswitch-active")
							? "_left"
							: "_right";
			...
			$.extend(
				{
					...
					_change: function (event) {
						...
						if (min === max) {
							$.data(thisSlider.get(0), "mobile-slider").handle.css(
								"z-index",
								1,
							);
							$.data(otherSlider.get(0), "mobile-slider").handle.css(
								"z-index",
								0,
							);
						} else {
		...
		$.widget(
			...
			classes = $el[0].className;
					...
					_create: function () {
						...
						var 
							...
							iconpos = options.icon
								? options.iconpos || this.select.jqmData("iconpos")
								: false,
							button = this.button
					...
					build: function () {
						...
						this.button.bind("vmousedown", () => {
							if (this.options.preventFocusZoom) {
								$.mobile.zoom.disable(true);
							}
						});
						this.label.bind("click focus", () => {
							if (this.options.preventFocusZoom) {
								$.mobile.zoom.disable(true);
							}
						});
						this.select.bind("focus", () => {
							if (this.options.preventFocusZoom) {
								$.mobile.zoom.disable(true);
							}
						});
						this.button.bind("mouseup", () => {
							if (this.options.preventFocusZoom) {
								setTimeout(() => {
									$.mobile.zoom.enable(true);
								}, 0);
					...
					setButtonCount: function () {
						...
						if (this.isMultiple) {
							this.buttonCount[selected.length > 1 ? "show" : "hide"]().text(
								selected.length,
							);
						}
					},
		...
		$.mobile.links = (target) => {
			...
			$(target)
				...
				.each(function () {
					...
					var 
						idref = this.getAttribute("href").substring(1);

					if (idref) {
						this.setAttribute("aria-haspopup", true);
						this.setAttribute("aria-owns", idref);
	...
	(($, undefined) => {
		...
		$.widget("mobile.popup", {
			...
			_themeClassFromOption: (prefix, value) => value
					? value === "none"
						? ""
						: prefix + value
					: prefix + "inherit",
			...
			_createPrerequisites: function (
				...
				prerequisites.screen.then(() => {
					if (prerequisites === this._prerequisites) {
						screenPrerequisite();
					}
				});
			...
			*/
			...
			open: function (options) {
				...
				this.window.one("beforenavigate", (theEvent) => {
					theEvent.preventDefault();
					this._open(options);
					this._bindContainerClose();
				});
		...
		$.mobile.document.on("pagebeforechange", (theEvent, data) => {
	...
	(($, undefined) => {
		var unfocusableItemSelector =
			...
			goToAdjacentItem = (item, target, direction) => {
		...
		$.widget("mobile.selectmenu", $.mobile.selectmenu, {
			...
			build: function () {
				...
				overlayThemeAttr = overlayTheme
					? " data-" + $.mobile.ns + "overlay-theme='" + overlayTheme + "'"
					: "";
				dividerThemeAttr =
			...
			_decideFormat: function () {
				var 
					...
					btnOffset = this.button.offset().top,
					screenHeight = $window.height();

				if (menuHeight > screenHeight - 80 || !$.support.scrollTop) {
					this.menuPage.appendTo($.mobile.pageContainer).page();
			...
			$.extend(
				{
					...
					_themeClassFromOption: (value) => value
							? value === "none"
								? ""
					...
					_setOptions: function (options) {
						...
						if (options.type !== undefined) {
							elem
								...
								.addClass(
									"ui-controlgroup-" +
										(options.type === "horizontal" ? "horizontal" : "vertical"),
								);
							callRefresh = true;
						}
	...
	(($, undefined) => {
		$.widget("mobile.toolbar", {
			...
			_addHeaderButtonClasses: function () {
				...
				this.rightbtn =
					this.rightbtn || headerAnchors.eq(1).addClass("ui-btn-right").length;
			},
	...
	(($, undefined) => {
		$.widget("mobile.toolbar", $.mobile.toolbar, {
			options: {
				...
				supportBlacklist: () => !$.support.fixedPosition,
			},
				...
				if (
					...
					if (o.fullscreen !== undefined) {
						...
						// If not fullscreen, add class to page to set top or bottom padding
							...
							$page
								.removeClass("ui-page-" + this.role + "-fullscreen")
				...
				page
					...
					.bind("focusin focusout", (e) => {
						...
						if (
							...
							!$(e.target).closest(".ui-header-fixed, .ui-footer-fixed").length
						) {
	...
	(($, undefined) => {
		$.widget("mobile.toolbar", $.mobile.toolbar, {
			...
			_viewportOffset: function () {
				...
				if (!header) {
					offset =
						Math.round(offset - this.window.height() + $el.outerHeight()) - 60;
				}
				return offset;
			},
			...
			_bindScrollWorkaround: function () {
				...
				this._on(this.window, {
					scrollstop: () => {
						var viewportOffset = this._viewportOffset();
						//check if the header is visible and if its in the right place
						if (viewportOffset > 2 && this._visible) {
							this._triggerRedraw();
						}
					},
				});
	...
	(($, undefined) => {
		$.widget("mobile.panel", {
			...
			_createModal: function () {
				var 
					target = this._parentPage
						? this._parentPage.parent()
						: this.element.parent();

				this._modal = $(
					"<div class='" + this.options.classes.modal + "'></div>",
				)
					.on("mousedown", () => {
						this.close();
					})
			...
			_positionPanel: function (scrollToTop) {
				var 
					panelInnerHeight = this._panelInner.outerHeight(),
					expand = panelInnerHeight > $.mobile.getScreenHeight();

				if (expand || !this.options.positionFixed) {
					if (expand) {
						this._unfixPanel();
			...
			_bindUpdateLayout: function () {
				...
				this.element.on("updatelayout", (/* e */) => {
					if (this._open) {
						this._positionPanel();
					}
				});
			...
			_bindSwipeEvents: function () {
				var 
					area = this._modal ? this.element.add(this._modal) : this.element;
			...
			_bindPageEvents: function () {
				...
				this.document
					...
					.on("panelbeforeopen", (e) => {
						if (this._open && e.target !== this.element[0]) {
							this.close();
						}
					})
					// On escape, close? might need to have a target check too...
					.on("keyup.panel", (e) => {
						if (e.keyCode === 27 && this._open) {
							this.close();
						}
					});
			...
			open: function (immediate) {
				if (!this._open) {
					var 
						o = this.options,
						_openPanel = () => {
							this._off(this.document, "panelclose");
							this._page().jqmData("panel", "open");
							...
							if (
								...
								this._wrapper.addClass(o.classes.animate);
								this._fixedToolbars().addClass(o.classes.animate);
							}

							if (!immediate && $.support.cssTransform3d && o.animate) {
								(this._wrapper || this.element).animationComplete(
									complete,
									"transition",
								);
							} else {
							...
							this._pageContentOpenClasses = this._getPosDisplayClasses(
								o.classes.pageContentPrefix,
							);

							if (o.display !== "overlay") {
								this._page().parent().addClass(o.classes.pageContainer);
								this._wrapper.addClass(this._pageContentOpenClasses);
								this._fixedToolbars().addClass(this._pageContentOpenClasses);
							}

							this._modalOpenClasses =
								this._getPosDisplayClasses(o.classes.modal) +
								" " +
								o.classes.modalOpen;
							if (this._modal) {
								this._modal
									...
									.height(
										Math.max(this._modal.height(), this.document.height()),
									);
							}
						},
						complete = () => {
							...
							if (o.display !== "overlay") {
								this._wrapper.addClass(o.classes.pageContentPrefix + "-open");
								this
									._fixedToolbars()
									.addClass(o.classes.pageContentPrefix + "-open");
							}

							this._bindFixListener();

							this._trigger("open");

							this._openedPage = this._page();
						};

					this._trigger("beforeopen");

					if (this._page().jqmData("panel") === "open") {
						this._on(this.document, {
				...
				if (this._open) {
					var 
						...
						_closePanel = () => {
							...
							if (o.display !== "overlay") {
								this._wrapper.removeClass(this._pageContentOpenClasses);
								this._fixedToolbars().removeClass(this._pageContentOpenClasses);
							}
							...
							if (this._modal) {
								this._modal.removeClass(this._modalOpenClasses).height("");
							}
						},
						complete = () => {
							if (o.theme && o.display !== "overlay") {
							...
							this.element.addClass(o.classes.panelClosed);

							if (o.display !== "overlay") {
								this._page().parent().removeClass(o.classes.pageContainer);
								this._wrapper.removeClass(
			...
			_destroy: function () {
				...
				if (o.display !== "overlay") {
					...
					if (
						otherPanels.not(".ui-panel-display-overlay").not(this.element)
							.length === 0
					) {
	...
	(($, undefined) => {
		...
		$.widget("mobile.filterable", {
			...
			_onKeyUp: function () {
				...
				if (search) {
					...
					this._timer = this._delay(function () {
						if (
							this._trigger("beforefilter", null, { input: search }) === false
						) {
							return false;
						}
			...
			_filterItems: function (val) {
				...
				if (hide.length === 0) {
					filterItems[
						opts.filterReveal && val.length === 0 ? "addClass" : "removeClass"
					]("ui-screen-hidden");
				} else {
	...
	(($, undefined) => {
		...
		var replaceSetOptions = (self, orig) => function (options) {
		...
		$.widget("mobile.filterable", $.mobile.filterable, {
			...
			_isSearchInternal: function () {
				return (
					this._search &&
					this._search.jqmData("ui-filterable-" + this.uuid + "-internal")
				);
			},
	...
	(($, undefined) => {
		...
		$.widget("ui.tabs", {
			...
			_tabId: (tab) => tab.attr("aria-controls") || "ui-tabs-" + getNextTabId(),

			_sanitizeSelector: (hash) => hash
					? hash.replace(/[!"$%&'()*+,./:;<=>?@[\]^`{|}~]/g, "\\$&")
					: "",
			...
			_ajaxSettings: function (anchor, event, eventData) {
				return {
					...
					beforeSend: (jqXHR, settings) => this._trigger(
							"beforeLoad",
							event,
